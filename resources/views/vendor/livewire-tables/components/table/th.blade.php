...
@if ($theme === 'tailwind')
  <th scope="col"
    ...
    @unless ($component->sortingIsEnabled() && ($column->isSortable() || $column->getSortCallback()))
      {{ $column->getTitle() }}
    @else
      <button wire:click="sortBy('{{ $column->isSortable() ? $column->getColumnSelectName() : $column->getSlug() }}')"
        ...
        <span class="relative flex items-center">
          @if ($direction === 'asc')
            <svg class="h-3 w-3 group-hover:opacity-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"></path>
            </svg>

            <svg class="absolute h-3 w-3 opacity-0 group-hover:opacity-100" fill="none" stroke="currentColor"
              viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
            </svg>
          @elseif ($direction === 'desc')
            <svg class="absolute h-3 w-3 opacity-0 group-hover:opacity-100" xmlns="http://www.w3.org/2000/svg"
          ...
          @else
            <svg class="h-3 w-3 opacity-0 transition-opacity duration-300 group-hover:opacity-100" fill="none"
  ...
  <th scope="col"
    ...
    @else
      <div class="d-flex align-items-center"
        ...
        <span class="d-flex align-items-center relative">
          @if ($direction === 'asc')
